...
//        print(self.beaconManager.monitoredRegions)
    ...
    func beaconManager(_ manager: Any, didEnter region: CLBeaconRegion) {
        if let user = Auth.auth().currentUser {
            self.ref.child("/users/\(user.uid)").observeSingleEvent(of: .value, with: { (usersnap) in
                ...
                if (userdata?["table"] as? NSInteger ?? 0) == 0 {
                    self.ref.child("/table/\(region.major!)").observeSingleEvent(of: .value, with: { (tablesnap) in
                        let value = tablesnap.value as? NSDictionary
                        ...
                        if ((number+1) <= (region.minor as! NSInteger)) {
                            ...
                            notification.alertBody = "歡迎光臨,\(user.displayName!)先生/小姐\n您正坐在\(region.identifier)"
    ...
    func beaconManager(_ manager: Any, didExitRegion region: CLBeaconRegion) {
        if let user = Auth.auth().currentUser {
            self.ref.child("/users/\(user.uid)").observeSingleEvent(of: .value, with: { (usersnap) in
                let userdata = usersnap.value as? NSDictionary
                if (userdata?["table"] as? NSInteger ?? 0) == (region.major as! NSInteger){
                    self.ref.child("/table/\(region.major!)").observeSingleEvent(of: .value, with: { (tablesnap) in
                        let value = tablesnap.value as? NSDictionary
                        var number = value?["taken"] as! NSInteger
                        if ((number-1) >= 0) {
                            ...
                            let notification = UILocalNotification()
                            notification.alertBody = "Goodbye,\(user.displayName!)先生/小姐\n期待您的再次光臨"
                            UIApplication.shared.presentLocalNotificationNow(notification)
                        }
                    })
                }
